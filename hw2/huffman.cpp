...
void compress(void)
{
	...
	for (int i=0;i<(huffman_binary.size()/8);i++)
	{
		...
		int len = eight_hufffman_code.size();
		//reverse the huffmancode
 		for (int m = 0; m<len/2; m++)
		{
  			//前后交换
  			char tmp_three = eight_hufffman_code[m];
  			eight_hufffman_code[m] = eight_hufffman_code[len-m-1];
  			eight_hufffman_code[len-m-1] = tmp_three;
 		}
		char piece[8];
		for (int i=0;i<8;i++)
		{
 		...
 		char tmp_one= dec_code%256;
		cout << hex <<tmp_one;
		/*
 		int frequency_to_print = dec_code;
		while (frequency_to_print!=0)
		{
			unsigned char tmp_one= frequency_to_print%256;
			cout << hex <<tmp_one;
			frequency_to_print = frequency_to_print/256;
		}
		*/
